...
- Convenciones de categoría del cliente. Para un VEP decidí por su Tipo de Pago y Concepto (campo `identificador`), nunca por la `sugerencia_tabla`: el concepto del banco es el mismo para todos los VEP.
  - VEP de "Empleadores SICOSS" (cargas sociales) y de "Retenciones Ganancias" (retenciones practicadas al personal) → `Personal - Leyes - {{nombre_corto}}`.
  - VEP de anticipos o saldo del impuesto a las Ganancias de la sociedad → `Impuesto - ARCA - Ganancias`.
  - VEP de IVA → `Impuesto - ARCA - IVA`. En un VEP consolidado, el tercer elemento es el impuesto de sus líneas (mirá las etiquetas de `importes`), no la palabra "consolidado".
  - Los pagos de ARBA (Ingresos Brutos) no son VEP de AFIP/ARCA: se rigen por la `sugerencia_tabla` (`Impuesto - ARBA - IIBB`).
  - Retiros de efectivo con tarjeta → `Pagos en efectivo`.
- Si un candidato trae `contexto_importe` (el importe aparece en una línea del texto del documento, por ejemplo un recibo dentro de un resumen de cuenta) y esa línea muestra un número de comprobante, usá ese número como `comprobante`.
...
# EJEMPLOS
Entrada: {"n":7,"concepto":"Transferencia realizada - A Ferretería Sur SRL / - fac / 30111111111","importe":-45320.5,"sugerencia_tabla":"Maquinaria - Ferretería - Ferretería Sur","candidatos":[{"doc":"doc_031","rol":"origen","emisor":"Ferretería Sur SRL","numero":"A 0003-00001234","importes":[{"etiqueta":"total","monto":45320.5}],"pertenece":"si","coincide_importe":true,"coincide_cuit":true}]}
...
Entrada: {"n":20,"concepto":"Debito automatico - Asociacion Ejemplo","importe":-718200.75,"sugerencia_tabla":"Asesoramiento - Asociación - Ejemplo","candidatos":[{"doc":"doc_053","rol":"origen","emisor":"Asociación Ejemplo","numero":"00025-00012345","importes":[{"etiqueta":"total","monto":715000.5}],"pertenece":"si","coincide_importe":false,"coincide_nombre":true,"suma_con":"doc_070"},{"doc":"doc_070","rol":"origen","emisor":"Asociación Ejemplo","numero":"00025-00012346","importes":[{"etiqueta":"total","monto":3200.25}],"pertenece":"si","coincide_importe":false,"coincide_nombre":true,"suma_con":"doc_053"}]}
Salida: {"n":20,"docs":["doc_053","doc_070"],"comprobante":"00025-00012345 ; 00025-00012346","detalle":"Asesoramiento - Asociación - Ejemplo","detalle_origen":"tabla","confianza":"alta","motivo":"dos facturas del mismo emisor suman el débito"}
